...
import { IntransitShipmentCard } from "./component/UnavailableShipmentCard/IntransitShipmentCard";
import { useInTransit } from "../../../services/api";

const UnavailableShipment: React.FC = () => {
  const [page, setPage] = useState(1);
  const limit = 5;
  const { ShipmentData, isLoading , refetch} = useInTransit(page, limit);
          ...
          {ShipmentData.length === 0 ? (
            ...
            <>
              <div className="flex flex-wrap gap-3">
                    {ShipmentData.map((shipment: { _id: string; name: string; status: string; trackingNo: string; origin: string; destination: string; }) => (
                  <IntransitShipmentCard
                    key={shipment._id}
                    _id={shipment._id}
...
export default UnavailableShipment;
