...
interface InTransitDetailsState {
  imageUrl: string;
  shipName: string;
  status: string;
  tno: string;
...
const InTransitDetails: React.FC = () => {
  ...
  const { imageUrl, shipName, status, tno, destination, location: currentLocation, _id } = location.state as InTransitDetailsState;
        ...
        <div className="flex flex-col md:flex-row border rounded-md shadow-md p-4 bg-white">
          ...
          <div className="w-full md:w-2/3 md:pl-4 flex flex-col justify-between">
            <div>
              ...
              <p className="text-gray-500 mt-2">Origin: <span className="text-black">{currentLocation}</span></p>
            </div>
            <div className="mt-4">
              <button
...
export default InTransitDetails;
