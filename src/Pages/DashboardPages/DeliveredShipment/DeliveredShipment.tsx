...
import { DeliveredShipmentCard } from "./component/DeliveredShipmentCard";
import { useCompleted } from "../../../services/api";
import { Shipment } from "../PendingShipment/PendingShipment";

const DeliveredShipment: React.FC = () => {
  const [page, setPage] = useState(1);
  const limit = 5;
  const { ShipmentData, isLoading, refetch } = useCompleted(page, limit);
  ...
  const getRandomDate = () => {
    ...
    const randomOffset = Math.floor(Math.random() * 30) + 1; // Random number between 1 and 30
    const futureDate = new Date(today.getFullYear(), today.getMonth(), today.getDate() + randomOffset);
    return futureDate.toLocaleDateString(); // Adjust format as needed
  };
